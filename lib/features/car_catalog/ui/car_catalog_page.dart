...
class _CarCatalogPageState extends State<CarCatalogPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await Modular.get<CarCatalogInteractor>().fetchCarBrands();
    });
  }

  @override
  Widget build(BuildContext context) {
    final interactor = Modular.get<CarCatalogInteractor>();
    ...
    return Scaffold(
      ...
      body: ValueListenableBuilder<CarCatalogState>(
        valueListenable: interactor,
        builder: (context, state, child) {
          return switch (interactor.value) {
                  ...
                  return Center(
                    child: Padding(
                      ...
                      child: TextButton(
                        ...
                        child: Text(
                          'Código: ${brand.code} | Marca: ${brand.name}',
                        ),
                      ),
                    ),
                  );
                ...
                itemBuilder: (_, index) {
                  ...
                  return Center(
                    child: Padding(
                      padding: const EdgeInsets.all(8),
                      child:
                          Text('Código: ${model.code} | Modelo: ${model.name}'),
                    ),
                  );
                },
              ),
